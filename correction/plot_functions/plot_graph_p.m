function plot_graph_p(ht_a, ft_a, ht_mpz, ft_mpz, ht_a_p, ft_a_p, ht_mpz_p, ft_mpz_p, case_n, fs, WAV)
% PLOT_GRAPH_P plots the frequency response graph of a filter and the
% frequency response graph of the same filter obtained from the modified
% version of its transfer function, together with its MPZ transformation.
%
% Input parameters:
...
%   ht_mpz_p :      Modified filter MPZ frequency response vector
%   ft_mpz_p :      Modified corresponding filter MPZ angular frequency vector
...
hold on;
legend('Analog', 'MPZ', 'Analog - Pole', 'MPZ - Pole');
...
if exist('WAV', 'var')
    ...
    hold on;
    legend('Analog', 'MPZ', 'Analog - Pole', 'MPZ - Pole', 'WAV');
